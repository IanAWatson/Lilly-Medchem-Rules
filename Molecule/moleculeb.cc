...
Molecule::add_molecule (const Molecule *m2)
{
  ...
  int m2atoms = m2->natoms();   // will call OK_MOLECULE (m2)
  ...
  for (int i = 0; i < m2atoms; i++)
  {
    ...
    if (copy_atom_based_user_specified_void_pointers_during_add_molecle)
...
// Check each chiral centre. the centre must be one of these atoms

//cerr << "Need to check " << nc << " chiral centres\n";
