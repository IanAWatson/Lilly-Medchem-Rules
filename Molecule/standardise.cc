...
// C1(=CC=C(F)C=C1)C1=C(N=NN1)C1=CC=NC=C1 000002134807
...
// atoms, those should be independent C12=C(C(=NC(=N1)O)O)NC=N2
...
// Only one remaining is lactam lactim
  ...
  for (int i = istart; i < _matoms; ++i)
  {
    ...
    for (int j = 0; j < attached_nitrogens; ++j)
    {
      ...
      if (bcn->is_double_bond() && bcn->part_of_cis_trans_grouping()) // C(=NNC(N)=N)(C=CC1=CC=C(O1)N(=O)=O)C(=O)NO MDDR238982 - shonw without cis trans bonds
        continue;
