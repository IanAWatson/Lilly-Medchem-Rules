...
Demerit::_add_demerit(const int increment, const const_IWSubstring & reason)
{
  ...
  if (0 == _demerit.number_elements() || increment >= _demerit.last_item()->demerit())
...
Demerit::do_sort()
{
  ...
  struct {
    int operator()(const Demerit_and_Reason * dr1, const Demerit_and_Reason * dr2) const
    {   
  ...
  _demerit.iwqsort(compareDemerit);
