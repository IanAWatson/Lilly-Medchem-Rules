...
int
Substructure_Ring_System_Specification::debug_print (ostream & os,
...
int
Substructure_Ring_System_Specification::terse_details (ostream & os,
                                      const IWString & indentation) const
{
  IWString ind = indentation;
  ind += "  ";
...
Substructure_Ring_System_Specification::matches (Molecule_to_Match & target)
{
  ...
  iw_auto_array<int> free_rtmp (rtmp);
                                                ...
                                                atom_number_t first_spinach_atom) const
{
  ...
  iw_auto_array<int> free_already_done (already_done);
                                                ...
                                                const int * in_ring_system) const
{
  ...
  for (int i = 0; i < matoms; i++)
  {
    ...
    for (int j = 0; j < acon; j++)
    {
      ...
      int d = shortest_distance_to_another_ring (target, already_done, k);

#ifdef DEBUG_SHORTEST_DISTANCE_TO_ANOTHER_RING
      cerr << "From atom " << i << " shortest dist to another ring " << d << endl;
#endif

      if (_distance_to_another_ring.matches (d))
        return 1;
