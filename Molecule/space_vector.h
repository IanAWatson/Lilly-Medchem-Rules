...
template <typename T> ostream & operator << (std::ostream &, const Space_Vector<T> &);

#if (IW_IMPLEMENTATIONS_EXPOSED) || defined(SPACE_VECTOR_IMPLEMENTATION)
...
template <typename T>
ostream &
operator << (ostream & os, const Space_Vector<T> & qq)
{
  os << "(" << qq.x () << "," << qq.y () << "," << qq.z () << ")";
