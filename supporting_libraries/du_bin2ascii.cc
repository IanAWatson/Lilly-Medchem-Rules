...
{s[0] = lookup[ static_cast<unsigned int>(c & 0xFC >> 2)].ascii;				\
 s[1] = lookup[static_cast<unsigned int>((c & 0x03) << 4 | ((d & 0xF0) >> 4))].ascii;	\
 s[2] = lookup[static_cast<unsigned int>((d & 0x0F) << 2 | ((e & 0xC0) >> 6))].ascii;	\
 s[3] = lookup[static_cast<unsigned int>(e & 0x3F)].ascii;				\
}

#define ASCII2BIN(s,c,d,e) \
{c = (lookup[static_cast<unsigned int>(s[0])].bin << 2) | ((lookup[static_cast<unsigned int>(s[1])].bin & 0x30) >> 4);	 \
 d = ((lookup[static_cast<unsigned int>(s[1])].bin & 0x0F) << 4) | ((lookup[static_cast<unsigned int>(s[2])].bin & 0x3C) >> 2);\
 e = ((lookup[static_cast<unsigned int>(s[2])].bin & 0x03) << 6) | lookup[static_cast<unsigned int>(s[3])].bin;		 \
}
...
/*  0   */ {'.', 0},
/*  1      {',', 0}, OLD way, 4.51 output */
/*  1   */ {'+', 0}, /* new way, 4.61 output, allows binary n-tuples */
/*  2   */ {'0', 0},
/*  3   */ {'1', 0},
/*  4   */ {'2', 0},
/*  5   */ {'3', 0},
...
/* 42 * */ {'e', 0},
/* 43 +  'f',  0, OLD way */
/* 43 + */ {'f', 1}, /* New way, allow either + or , on input */
/* 44 , */ {'g', 1},
/* 45 - */ {'h', 0},
/* 46 . */ {'i', 0},
/* 47 / */ {'j', 0},
...
char *du_bin2ascii(int *palen, int blen, char *b)
{
...
#ifdef DAYLIGHT_ORIGINAL_VERSION

char *du_ascii2bin(int *pblen, int alen, char *ascii)
{
  char *binary, *p;
  int i;
  ...
  /**** Compute the binary array's length and allocate it ****/
  *pblen = (alen - 1) / 4;
  *pblen *= 3;
  if (0 == *pblen)
...
/*
  Lots of problems with this. Turns out that encoding 256 bits
  actually requires 258 bytes - because that's the next multiple
  of 2 above 256. But we want to only use 256 bytes
*/
             ...
             unsigned char * binary, unsigned int & nbytes)
{
  ...
  int valid_bytes_in_last_3 = ascii[nchars - 1] - '0';

  unsigned int istop;
  if (3 == valid_bytes_in_last_3)
    istop = nbytes;
  else
   istop = nbytes - 3;

  for (unsigned int i = 0; i < istop; i += 3) {
    ASCII2BIN(ascii, binary[i],  binary[i+1], binary[i+2]);
    ascii += 4;
  }
  ...
  if (1 == valid_bytes_in_last_3)
  {
    ...
    ASCII2BIN (ascii, binary[nbytes - 3], notused1, notused2);
    nbytes -= 2;
  }
  else if (2 == valid_bytes_in_last_3)
  {
