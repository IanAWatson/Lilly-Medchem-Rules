...
iwstring_data_source::_setup_stream (const char * fname)
{
	...
	else if (0 == strncmp (fname + strlen (fname) - 3, ".gz", 3))
	{
...
int
iwstring_data_source::debug_print (ostream & os) const
{
	assert (os.good());
...
iwstring_data_source::_read_more_data_into_read_buffer()
{
	...
	if(_isstringbuffer)
	{
		if(!_isstringbuffer_loaded)
		{
			...
			strcpy(_read_buffer, _stringbuffer);
...
iwstring_data_source::tellg() const
{
	...
	else
	{
		...
		if (current_offset < static_cast<off_t> (0))
		{
...
int
iwstring_data_source::seekg (off_t zoffset,
							 int whence)
{
...
iwstring_data_source::file_size()
{
	...
	off_t current_position = IW_FD_LSEEK (_fd, 0, SEEK_CUR);

	off_t rc = IW_FD_LSEEK (_fd, 0, SEEK_END);

	off_t back_again = IW_FD_LSEEK (_fd, current_position, SEEK_SET);
...
int
iwstring_data_source::_write_read_buffer (ostream & output,
										  off_t & nbytes)
{
	int chars_to_write;
	if (static_cast<off_t> (_chars_in_read_buffer) >= nbytes)
...
int
iwstring_data_source::echo (ostream & output, off_t nbytes)
{
	assert (ok());
...
int
iwstring_data_source::echo (IWString_and_File_Descriptor & output, off_t nbytes)
{
	assert (ok());
...
int
iwstring_data_source::echo_records (ostream & os, int necho)
{
	assert (ok());
	assert (necho >= 0);
...
// arch-tag: 1d46077f-9184-46b7-b53a-900214caf38e
